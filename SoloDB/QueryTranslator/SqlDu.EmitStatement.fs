...
let emitInsert (ctx: EmitContext) (ins: InsertStatement) : Emitted =
    ...
    let rowsSql =
        ...
        |> List.map (fun row ->
            ...
            let rowParams = rowParts |> List.collect (fun e -> e.Parameters)
            allParams <- allParams @ rowParams
            sprintf "(%s)" rowSql)
        |> String.concat ", "
    ...
    | Some returningExprs ->
        ...
        let retParams = retParts |> List.collect (fun e -> e.Parameters)
        sql <- sprintf "%s RETURNING %s" sql retSql
        allParams <- allParams @ retParams
    | None -> ()
